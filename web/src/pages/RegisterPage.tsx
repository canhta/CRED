...
export function RegisterPage({
  ...
  // An invite bypasses the bootstrap gate entirely -- the health check
  // below only decides whether *open* (no-invite) registration is still
  // available.
  if (!invite) {
    if (health.isLoading) {
      return null;
  ...
  const submit = () => {
    register.mutate({ email, password, invite }, { onSuccess });
  };
    ...
    <Center height="100dvh">
      <Card padding={4} width={360}>
        <VStack gap={4}>
          <VStack gap={1}>
            <Heading level={4}>
              {invite ? 'Create your account' : 'Create the first account'}
            </Heading>
            <Text type="body" color="secondary">
              {invite
                ? "You've been invited to join this CRED console."
