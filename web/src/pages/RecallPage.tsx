  ...
  return (
    <Card variant="muted" padding={3}>
      <HStack gap={6} vAlign="center" wrap="wrap">
        <Stat label="Candidates" value={String(data.candidates)} />
        <Stat label="Authorized" value={String(data.authorized)} />
        <Stat
          label="Dominant arm"
          value={
            data.dominant_arm ? (
        ...
        <Stat
          label="Total latency"
          value={`${data.timings.total_ms.toFixed(1)} ms`}
        />
        {data.omitted_for_budget > 0 ? (
          <Stat
            label="Omitted for budget"
            value={
              <Badge
                variant="warning"
