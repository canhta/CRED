...
// RegisterRequest is the body of POST /api/auth/register. Invite is empty
// for the open bootstrap path; when set, register() redeems it instead of
// checking whether registration is still open.
...
// CreateInviteRequest is the body of POST /api/team/invites.
type CreateInviteRequest struct {
	Email string `json:"email" binding:"required,email"`
	Role  string `json:"role" binding:"required,oneof=admin member"`
}

// CreateInviteResponse is the body of a successful POST /api/team/invites --
// the only time the raw invite token is visible; only its hash is ever
// stored.
...
// Invite is one row of GET /api/team/invites -- a pending, still-redeemable
// invite.
type Invite struct {
	ID        string `json:"id"`
	Email     string `json:"email"`
...
// TeamMember is one row of GET /api/team/members.
