...
These load automatically when you touch matching files. They are not imported
here, to avoid double-loading them.

- `.claude/rules/docs.md` — documentation and evidence standards. Matches
  `docs/**`, `README.md`, `AGENTS.md`, `CLAUDE.md`.
- `.claude/rules/go.md` — Go conventions, and the design laws that are enforced
  structurally rather than stated. Matches `**/*.go`, `go.mod`, `.golangci.yml`,
  `.github/workflows/**`.

**`CLAUDE.md` is canonical; `AGENTS.md` is a symlink to it.** A symlink keeps
the two tools' expected filenames pointing at one file on disk, so there is no
import step and no risk of the two drifting apart. Edit `CLAUDE.md`.
