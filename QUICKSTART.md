...
The console needs an account before it shows anything: on an empty database the
first visit redirects to **Register**, and whatever you register there becomes
the admin. For a one-command dev account instead of clicking through the form:

```sh
task db:reset   # fresh Postgres container + migrations (destroys local data)
task db:seed    # registers admin@cred.local / cred-dev-password
```
...
task db:reset       # fresh local Postgres container + migrations
task db:seed        # register the dev console account (admin@cred.local / cred-dev-password)
```
